...
public class SingleRepository implements Repository {
    ...
    public void logPrgStateExec(PrgState state){
        try {
            ...
            Set<Integer> heaps = heap.getDict().getKeys();
            for (Integer key: heaps) {
                logFile.append(key.toString());
                logFile.append("->");
                logFile.append(heap.getDict().lookup(key).toString());
                logFile.append("\r\n");
            }

            logFile.append("SemaphoreTable:\r\n");
            Semaphore s = state.getSemaphore();
            Set<Integer> ss = s.getDict().getKeys();
            for (Integer key: ss) {
                logFile.append(key.toString());
                logFile.append("->");
                logFile.append(s.getDict().lookup(key).toString());
                logFile.append("\r\n");
            }
